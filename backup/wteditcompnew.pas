...
//  TBeforeFind =   procedure (Sender: Tobject;var CampiValori,CampiWhere, CampiJoin: string; var CheckFiltro:Boolean = False; var Indice:string = '';Var SelectCustomer:string = '') of object;
  ...
  TBeforeFind =   procedure (Sender: Tobject;var CampiValori,CampiWhere, CampiJoin: string; var CheckFiltro:Boolean; var Indice:string;Var SelectCustomer:string) of object;
...
// {dsRefreshFields}   'Refresh Campo');
 ...
 type
  ...
  public
    ...
    procedure    DoBeforeFind(var CampiValori,CampiWhere, CampiJoin: string; var CheckFiltro:Boolean; var Indice:string; Var SelectCustomer:string);
...
(*procedure TwtEditCompNew.setActive(AValue: booleaN);
Var x:integer;
begin
...
begin
 ...
 CampiJoin:='';
 filtro:= '';
 DoBeforeFind(CampiValori, CampiWhere, CampiJoin, CheckFiltro, Indice, SelectCustomer);
...
begin
  if Assigned(FBeforeFind) then
    begin
      CheckFiltro := False;
      FBeforeFind(self,CampiValori, CampiWhere, CampiJoin, CheckFiltro, Indice, SelectCustomer);
    end;
end;
   ...
   begin
    ...
    if  AtProcedure in Items[PosTab].attrib then
      begin
       ...
       if (Items[PosTab].ClassName = 'TWTCheckBox') then
        begin
         if (TWTCheckBox(Owner.Components[Items[PosTab].index]).Checked) then
           ValueField:= 'T'
         else
           ValueField:= 'F'
        end
       // se il componente è di tipo ComboBox
       else if Items[PosTab].ClassName = 'TWTComboBox' then
         ValueField:= IntToStr(TWTComboBox(Owner.Components[Items[PosTab].index]).ItemIndex)
       else if (Items[PosTab].ClassName = 'TWTComboBoxSql') then
         ValueField:=  TWTComboBoxSql(Owner.Components[Items[PosTab].index]).Text
       else
         ValueField:=  TCustomEdit(Owner.Components[Items[PosTab].index]).Text;
       if ValueField = '  -  -    ' then ValueField := '';
        if Trim(ValueField) <> '' then
         begin
           // se il componente e di tipo TWTComboBoxSql
           if Items[PosTab].ClassName = 'TWTComboBoxSql'  then
               valori:= valori + '''' +
               TWTComboBoxSql(Owner.Components[Items[PosTab].index]).ValueLookField + ''','

           else if Items[PosTab].ClassName = 'TWDateEdit'  then
               valori:= valori + '''' +
               FormatDateTime('DD.MM.YYYY',TWDateEdit(Owner.Components[Items[i].index]).Date) + ''','
           else if Items[PosTab].ClassName = 'TumDataEdit'  then
               valori:= valori + '''' +
                       TumDataEdit(Owner.Components[Items[PosTab].index]).ValueIB+ ''','

           else if Items[PosTab].ClassName = 'TumNumberEdit'  then //sostituisco la virgola con il punto
